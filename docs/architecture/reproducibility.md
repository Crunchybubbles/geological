...
Generated colluvium evaluates immutable source-capacity tranches from the deterministic weathering depth, slope, distance, and resolved erodibility at each bounded sample. `StrictMath` response functions and fixed-unit rounding partition each input exactly into retained and mobilized mass, then partition mobilized mass into transport loss, bypass, and deposition. A canonical largest-remainder reduction converts deposited mass to exact one-million-part shares. Those same shares blend constituent modes and registry-authored grain yields, from which texture and physical-property state follow. Every response input, allocation, delivered share, and resulting state joins the canonical colluvial body-ID purpose. No mutable draw, traversal state, or query order enters this derivation.
