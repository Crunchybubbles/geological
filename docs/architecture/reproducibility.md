...
Adaptive colluvial routes expose their nominal routed distance, endpoint straight-line distance, and bounded route-directness index. The path response multiplies continuity by that index, so geometric detours reduce transport efficiency deterministically while a zero-reach local path remains direct. These diagnostics and all per-reach routing decisions are derived from immutable terrain samples and are included in the causal body-ID purpose.
