...
| 4 — Minecraft vertical slice | Platform-neutral canonical Overworld/Nether/End profile identities plus deterministic seed/profile/chunk identity, 16×16 target bounds, logical stage order, stage-keyed streams, immutable worldgen snapshots, supplied-executor context, and chunk-local write contract are checked in. A separate `neoforge-adapter` now pins the exact 1.21.1/21.1.249/ModDevGradle 2.0.146 toolchain, loads the mod, and deterministically bridges a Minecraft dimension/chunk callback into the core request contract; no terrain or block placement yet. | 3–6 slices: Overworld terrain/material stages, clipping/caches, palette, commands/debug traces, compatibility, and benchmarks. |
...
are now complete as a minimal loader slice. The immediate engineering slice is a custom world
preset/generator hook that can consume the frozen request and snapshot contracts for Overworld
terrain controls; it must still stop short of block palette writes until those controls are tested.
