...
| 6 — secondary expansion | `phase6-alpha.2` projects both the gated Phase 3 gossan/oxidation/supergene Cu profile and source-gated bauxite/Ni-Co laterite profiles into world-column overlays with province-frame transforms, fixed-point source budgets, barren behavior, chunk seams, `/geology secondary` and `/geology laterite` views, and deterministic four-chunk review artifacts. | 2–4 slices: additional cassiterite/heavy-mineral/diamond placers, karst/regolith/paleosurfaces, and optional glacial transport. |
