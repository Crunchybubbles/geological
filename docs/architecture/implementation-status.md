...
| 3 — mineral systems | Six architecture families, porphyry fluid/metal zoning, gated supergene refinement, and a provisional six-dataset validation contract are implemented. | 2–4 slices: replace provisional anchors with audited source tables, complete held-out quantile/covariance reports, and refine non-concentric geometry. |
...
The immediate next engineering slice is the Phase 3 raw-table audit and held-out validation
report. The checked-in `MineralSystemValidationReport` rows are marked
`SOURCE_ANCHORS_PROVISIONAL`; they establish the metadata, missing/censoring, budget, and
deterministic-report contracts without claiming that a small anchor table is an unbiased or
redistributable empirical population.
