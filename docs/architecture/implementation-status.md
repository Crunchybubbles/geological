...
| 7 — deposit-family expansion | `phase7-alpha.4` adds the greisen residual-fluid proxy, a source-gated skarn carbonate-contact fixture, a shallow epithermal fluid-path projection, and a deformation/metamorphic-fluid orogenic-gold projection around the existing engines, with bounded horizons, closed source/release/loss/deposit accounting, explicit actual-host-only skarn behavior, chunk seams, `/geology greisen`, `/geology skarn`, `/geology epithermal`, `/geology orogenic-gold`, and deterministic review artifacts. | 3+ family slices, each with dossier, data, invariants, calibration, and clues. |
