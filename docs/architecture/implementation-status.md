...
| 5 — exploration geology | Phase 5 alpha.3 provides transient deterministic outcrop/float/contact/structural observations, bounded hand-sample identification, and soil/stream/heavy-mineral sampling with coarse constituent/indicator projections, source provenance, drainage/trap context, stable IDs, and read-only `/geology observations`, `/geology hand-sample`, `/geology soil`, `/geology stream-sediment`, and `/geology heavy-mineral`; no persistence or hidden-truth storage is used. | 2–4 slices: uncertainty/detection limits, drilling/cross-sections, notebook/map persistence, and Phase 5 telemetry/exit review. |
