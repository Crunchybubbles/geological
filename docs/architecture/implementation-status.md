...
| 5 — exploration geology | Phase 5 alpha.6 provides transient deterministic outcrop/float/contact/structural observations, bounded hand-sample and soil/stream/heavy-mineral sampling, interval-valued geochemical anomaly estimates, contiguous drill-core logs, and vertical cross-section traces with stable IDs, provenance, confidence/detection evidence, and read-only `/geology observations`, `/geology hand-sample`, `/geology soil`, `/geology stream-sediment`, `/geology heavy-mineral`, `/geology anomaly`, `/geology drill`, and `/geology vertical-section`; no persistence or hidden-truth storage is used. | 1–2 slices: notebook/map persistence and Phase 5 telemetry/exit review. |
