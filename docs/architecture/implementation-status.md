...
| 5 — exploration geology | Phase 5 alpha.4 provides transient deterministic outcrop/float/contact/structural observations, bounded hand-sample identification, soil/stream/heavy-mineral sampling, and interval-valued geochemical anomaly estimates with method-specific detection limits, censoring, stable IDs, provenance, and read-only `/geology observations`, `/geology hand-sample`, `/geology soil`, `/geology stream-sediment`, `/geology heavy-mineral`, and `/geology anomaly`; no persistence or hidden-truth storage is used. | 2–3 slices: drilling/cross-sections, notebook/map persistence, and Phase 5 telemetry/exit review. |
