...
| 6 — secondary expansion | `phase6-alpha.4` projects the gated Phase 3 gossan/oxidation/supergene Cu profile, source-gated bauxite/Ni-Co laterite profiles, cassiterite/heavy-mineral/diamond placer families, and typed present/buried/karst paleosurface refinements into world-column overlays with province-frame transforms, fixed-point source budgets where applicable, barren behavior, chunk seams, `/geology secondary`, `/geology laterite`, `/geology placers`, and `/geology paleosurface` views, and deterministic four-chunk review artifacts. | 0–1 slice: optional glacial transport only if an explicit ice-history/source contract is authored. |
