...
| 7 — deposit-family expansion | `phase7-alpha.1` adds a source-gated greisen residual-fluid proxy around an evolved felsic pulse, with bounded quartz-muscovite, tourmaline-proxy, and kaolinitic-margin horizons, closed source/release/loss/deposit accounting, chunk seams, `/geology greisen`, and a deterministic review artifact. | 6+ family slices, each with dossier, data, invariants, calibration, and clues. |
