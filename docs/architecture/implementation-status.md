...
| 5 — exploration geology | Phase 5 alpha.5 provides transient deterministic outcrop/float/contact/structural observations, bounded hand-sample and soil/stream/heavy-mineral sampling, interval-valued geochemical anomaly estimates, and bounded contiguous drill-core logs with stable IDs, provenance, confidence/detection evidence, and read-only `/geology observations`, `/geology hand-sample`, `/geology soil`, `/geology stream-sediment`, `/geology heavy-mineral`, `/geology anomaly`, and `/geology drill`; no persistence or hidden-truth storage is used. | 1–2 slices: vertical cross-sections, notebook/map persistence, and Phase 5 telemetry/exit review. |
