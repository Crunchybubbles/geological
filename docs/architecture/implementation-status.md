...
| 3 — mineral systems | Six architecture families, porphyry fluid/metal zoning, gated supergene refinement, and provisional six-dataset validation plus deterministic held-out quantile/covariance projections are implemented. | 2–3 slices: replace provisional anchors with audited source tables, promote source-backed held-out statistics, and refine non-concentric geometry. |
...
The immediate next engineering slice is the Phase 3 raw-table audit and source-backed held-out
validation report. The checked-in `MineralSystemValidationReport` rows are marked
`SOURCE_ANCHORS_PROVISIONAL`; they establish the metadata, missing/censoring, budget, and
deterministic quantile/covariance report contracts without claiming that a small anchor table is
an unbiased or redistributable empirical population.
