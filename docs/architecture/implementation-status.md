...
| 3 — mineral systems | Six architecture families, porphyry fluid/metal zoning, gated supergene refinement, audited porphyry/VMS/LCT/potash/BIF/placer source subsets, deterministic held-out quantile/covariance projections, and a bounded non-concentric porphyry footprint are implemented. | 1 sign-off slice: promote full-population statistics where the source supports redistribution. |
...
Porphyry, VMS, LCT, evaporite/potash, BIF, and placer now have checked-in
`RAW_TABLE_AUDITED_SUBSET` resources. The remaining
`MineralSystemValidationReport` rows use the subset-audit status. Together they establish the
metadata, missing/censoring, budget, and deterministic quantile/covariance report contracts
without claiming that a small subset is an unbiased or redistributable full population.
