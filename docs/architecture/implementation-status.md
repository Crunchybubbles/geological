...
| 8 — Nether/End profiles | `phase8-alpha.6` adds the profile-locked Nether thermal/magmatic province, material history, and source-linked quartz/gold/glowstone/ancient-debris prototypes, plus an End parent-body/fragment compiler with primitive, silicate-differentiated, metal-separated, and previously-melted families, central/gateway/outer roles, void gaps, impact/breccia, regolith ledgers, and a protected central/gateway/outer progression contract. The dimension-aware debug trace routes each canonical dimension through its native compiler, preserves distinct seed/profile/chunk identities, and proves adapter-boundary seam/topology invariants. NeoForge now binds custom Nether and End generator codecs into the selected world preset and writes native host/void/fluid plans without vanilla carvers or surface replacement. Deterministic `netherThermal`, `netherResources`, `endFragments`, `endProgression`, and `dimensionTraces` artifacts prove bounded identities and seams. | 1–3 slices: cross-dimensional compatibility/lore review and expert sign-off. |
...
models. Cross-dimensional compatibility/lore review and expert sign-off remain separate slices.
