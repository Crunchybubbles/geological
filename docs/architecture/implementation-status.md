...
| 3 — mineral systems | Six architecture families, porphyry fluid/metal zoning, gated supergene refinement, audited porphyry/VMS/LCT/potash subsets plus complete 66-row BIF and 83-row placer tables, deterministic held-out quantile/covariance projections, and a bounded non-concentric porphyry footprint are implemented. | 1 sign-off slice: review redistribution for the remaining subset families where the source supports it. |
...
Porphyry, VMS, LCT, and evaporite/potash have checked-in `RAW_TABLE_AUDITED_SUBSET` resources;
BIF and placer now have complete 66-row and 83-row `RAW_TABLE_AUDITED` resources. Together they establish
