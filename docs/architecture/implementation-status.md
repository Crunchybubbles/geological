...
| 9 — comprehensive geochemistry | `phase9-alpha.5` centralizes the 23-element/three-sulfur-state crystal-capture responses in a versioned sparse catalog, wires the magma residual split to it, and exposes confidence/review status in the artifact. Rows remain authored proxies pending external review. | 1–2 slices: external scientific/license review of response data and a processing-facing assay/mineral-liberation API. |
