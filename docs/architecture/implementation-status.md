...
| 7 — deposit-family expansion | `phase7-alpha.2` adds the greisen residual-fluid proxy and a source-gated skarn carbonate-contact fixture around the existing magmatic-hydrothermal engine, with bounded prograde/retrograde horizons, closed source/release/loss/deposit accounting, explicit actual-host-only default behavior, chunk seams, `/geology greisen`, `/geology skarn`, and deterministic review artifacts. | 5+ family slices, each with dossier, data, invariants, calibration, and clues. |
