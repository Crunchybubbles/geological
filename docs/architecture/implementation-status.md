...
| 4 — Minecraft vertical slice | Platform-neutral canonical Overworld/Nether/End profile identities plus a deterministic seed/profile/chunk identity, 16×16 target bounds, logical stage order, stage-keyed streams, and chunk-local write contract are checked in; no NeoForge adapter or block placement yet. | 5–8 slices: NeoForge lock/adapter, Overworld terrain/material stages, clipping/caches, palette, commands/debug traces, compatibility, and benchmarks. |
