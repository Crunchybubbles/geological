...
| 9 — comprehensive geochemistry | `phase9-alpha.2` expands the typed element vocabulary to 50 entries, adds condition-qualified profiles for the extension, accepts the expanded pathfinder set in sparse trace vectors, and partitions newly tracked incompatible/residual elements with exact closure. Composition rounding remains limited to authored elements, so existing recipes do not gain invented mass. | 2–4 slices: solid-solution/polymorph refinement, isotopic/provenance additions, reviewed response/partition datasets, and processing-facing assay/mineral-liberation APIs. |
