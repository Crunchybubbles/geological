...
| 3 — mineral systems | Six architecture families, porphyry fluid/metal zoning, gated supergene refinement, one audited porphyry source subset, and deterministic held-out quantile/covariance projections are implemented. | 2–3 slices: audit the remaining source tables, promote full-population statistics, and refine non-concentric geometry. |
...
Porphyry now has a checked-in `RAW_TABLE_AUDITED_SUBSET`; the other
