...
and applies a separate regolith palette during `buildSurface`; custom palette registration and
map/section debug overlays remain separate. The generator/preset slice now registers a real
...
is map/section debug overlays plus compatibility/benchmark integration around the deterministic
command/debug trace without allowing vanilla surface rules to overwrite geology.
