...
| 6 — secondary expansion | `phase6-alpha.3` projects the gated Phase 3 gossan/oxidation/supergene Cu profile, source-gated bauxite/Ni-Co laterite profiles, and cassiterite/heavy-mineral/diamond placer families into world-column overlays with province-frame transforms, fixed-point source budgets, barren behavior, chunk seams, `/geology secondary`, `/geology laterite`, and `/geology placers` views, and deterministic four-chunk review artifacts. | 1–2 slices: karst/regolith/paleosurface refinement and optional glacial transport. |
