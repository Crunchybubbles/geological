...
| 5 — exploration geology | Phase 5 alpha.2 provides transient deterministic outcrop/float/contact/structural observations plus bounded hand-sample identification with visible constituent modes, assay-needed signaling, stable IDs, provenance, confidence, target-chunk planning, and read-only `/geology observations` and `/geology hand-sample`; no persistence or hidden-truth storage is used. | 3–5 slices: soil/stream/heavy-mineral sampling, uncertainty/detection limits, drilling/cross-sections, and notebook/map persistence. |
