...
| 7 — deposit-family expansion | `phase7-alpha.3` adds the greisen residual-fluid proxy, a source-gated skarn carbonate-contact fixture, and a shallow epithermal fluid-path projection around the existing magmatic-hydrothermal engine, with bounded horizons, closed source/release/loss/deposit accounting, explicit actual-host-only skarn behavior, chunk seams, `/geology greisen`, `/geology skarn`, `/geology epithermal`, and deterministic review artifacts. | 4+ family slices, each with dossier, data, invariants, calibration, and clues. |
