...
| 3 — mineral systems | Six architecture families, porphyry fluid/metal zoning, gated supergene refinement, an audited porphyry subset plus complete 608-row VMS, 86-row LCT, 66-row BIF, 102-row qualifying potash, and 83-row placer tables, deterministic held-out quantile/covariance projections, and a bounded non-concentric porphyry footprint are implemented. | 1 sign-off slice: review redistribution for the remaining porphyry subset where the source supports it. |
...
Porphyry has a checked-in `RAW_TABLE_AUDITED_SUBSET` resource; VMS, LCT, BIF, potash, and placer
now have complete 608-row, 86-row, 66-row, 102-row qualifying, and 83-row `RAW_TABLE_AUDITED` resources. Together they establish
