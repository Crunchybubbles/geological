...
| 5 — exploration geology | Phase 5 alpha.1 provides transient deterministic outcrop/float/contact/structural observations with stable IDs, provenance, confidence/scale fields, target-chunk planning, and read-only `/geology observations`; no persistence or hidden-truth storage is used. | 4–6 slices: hand-sample identification, soil/stream/heavy-mineral sampling, uncertainty/detection limits, drilling/cross-sections, and notebook/map persistence. |
