...
| 7 — deposit-family expansion | `phase7-alpha.5` adds the greisen residual-fluid proxy, a source-gated skarn carbonate-contact fixture, a shallow epithermal fluid-path projection, a deformation/metamorphic-fluid orogenic-gold projection, and separate MVT/SEDEX/sediment-hosted-copper basin/redox projections around the existing engines, with bounded horizons, closed source/release/loss/deposit accounting, explicit actual-host-only fixtures, chunk seams, `/geology greisen`, `/geology skarn`, `/geology epithermal`, `/geology orogenic-gold`, `/geology basin-hydrothermal`, and deterministic review artifacts. | 2+ family slices, each with dossier, data, invariants, calibration, and clues. |
