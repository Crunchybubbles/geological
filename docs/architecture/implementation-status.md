...
| 9 — comprehensive geochemistry | `phase9-alpha.1` adds `ElementBehaviorCatalog`, a condition-qualified affinity/mobility/host registry for every current element, with explicit volatile/radiogenic flags and material-review artifact exposure. Existing sparse vectors and exact ledgers remain unchanged. | 3–5 slices: expanded element vocabulary/source profiles, solid-solution/polymorph refinement, isotopic/provenance additions, reviewed response/partition datasets, and processing-facing assay/mineral-liberation APIs. |
