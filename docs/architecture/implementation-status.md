...
| 5 — exploration geology | Phase 5 alpha.7 provides transient deterministic outcrop/float/contact/structural observations, bounded hand-sample and soil/stream/heavy-mineral sampling, interval-valued geochemical anomaly estimates, contiguous drill-core logs, vertical cross-section traces, and world-persistent per-player notebooks/maps containing only player-visible evidence. Stable IDs, frozen world identity, provenance, confidence/detection evidence, bounded NBT, deterministic notebook digests, and `/geology observations`, `/geology hand-sample`, `/geology soil`, `/geology stream-sediment`, `/geology heavy-mineral`, `/geology anomaly`, `/geology drill`, `/geology vertical-section`, and `/geology notebook` paths are covered by deterministic tests. | 1 sign-off slice: Phase 5 telemetry/exit review measuring clue sufficiency and travel burden. |
