...
Phase 7 alpha.4 now adds a source-gated greisen residual-fluid proxy around an evolved felsic pulse,
an explicit carbonate-contact skarn fixture, a shallow epithermal fluid-path projection, and a
deformation/metamorphic-fluid orogenic-gold projection through `/geology greisen`, `/geology skarn`,
`/geology epithermal`, `/geology orogenic-gold`, and the matching review tasks. These
quartz-muscovite, tourmaline-proxy, calc-silicate, retrograde, silica, argillic, and
quartz-carbonate horizons are bounded projections rather than assays or voxel-grade inventories;
skarn's default planner remains actual-host-only, and artifacts record closed proxy ledgers and
...
- `neoforge-adapter` is the pinned NeoForge 21.1.249 loader boundary. It contains the mod entry
  ...
  projection, bounded Phase 6 secondary-weathering, laterite, secondary-placer, paleosurface, and opt-in glacial overlays, the Phase 7 greisen residual-fluid proxy, actual-host-only skarn, shallow epithermal, and metamorphic-fluid orogenic-gold overlays, read-only `/geology` column/map/section/vertical-section/observation/hand-sample/sampling/anomaly/drill overlays, and the
...
See [the Phase 0 architecture](docs/architecture/phase-0-atlas.md), [Phase 1 query-core increment](docs/architecture/phase-1-query-core.md), [Phase 2 material-state increment](docs/architecture/phase-2-material-state.md), [Phase 5 exploration increment](docs/architecture/phase-5-exploration.md), [Phase 6 secondary-systems increment](docs/architecture/phase-6-secondary-systems.md), [Phase 7 deposit-family increment](docs/architecture/phase-7-deposit-families.md), [registry authoring contract](docs/architecture/registry-authoring.md), [reproducibility contract](docs/architecture/reproducibility.md), and [toolchain policy](docs/development/toolchain.md) before extending the model.
