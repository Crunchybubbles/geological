...
Phase 6 alpha.2 now projects the gated primary-Cu gossan/oxidation/supergene profile plus
source-gated bauxite/Ni-Co laterite profiles into bounded world-column overlays through
`/geology secondary` and `/geology laterite`; `secondaryWeathering` and `laterite` record
source-budget closure and adjacent-chunk seam stability in deterministic review artifacts.
...
On Windows, use `gradlew.bat` in place of `./gradlew`. Maps, cross-sections, column plans, JSON traces, and measurements are written below `atlas-cli/build/phase1/`; the deterministic material/catalog/reservoir review is written below `atlas-cli/build/phase2/`; the Phase 4 worldgen observation is written below `atlas-cli/build/phase4/worldgen/`; the Phase 5 exploration telemetry is written below `atlas-cli/build/phase5/exploration/`; the Phase 6 secondary-weathering review is written below `atlas-cli/build/phase6/secondary/`, and the laterite review below `atlas-cli/build/phase6/laterite/`.
...
- `neoforge-adapter` is the pinned NeoForge 21.1.249 loader boundary. It contains the mod entry
  ...
  projection, bounded Phase 6 secondary-weathering and laterite overlays, read-only `/geology` column/map/section/vertical-section/observation/hand-sample/sampling/anomaly/drill overlays, and the
...
See [the Phase 0 architecture](docs/architecture/phase-0-atlas.md), [Phase 1 query-core increment](docs/architecture/phase-1-query-core.md), [Phase 2 material-state increment](docs/architecture/phase-2-material-state.md), [Phase 5 exploration increment](docs/architecture/phase-5-exploration.md), [Phase 6 secondary-systems increment](docs/architecture/phase-6-secondary-systems.md), [registry authoring contract](docs/architecture/registry-authoring.md), [reproducibility contract](docs/architecture/reproducibility.md), and [toolchain policy](docs/development/toolchain.md) before extending the model.
