...
Phase 6 alpha.1 now projects the gated primary-Cu gossan/oxidation/supergene profile into bounded
world-column overlays through `/geology secondary`; `secondaryWeathering` records source-budget
closure and adjacent-chunk seam stability in a deterministic review artifact.
...
- `neoforge-adapter` is the pinned NeoForge 21.1.249 loader boundary. It contains the mod entry
  ...
  projection, bounded Phase 6 secondary-weathering overlay, read-only `/geology` column/map/section/vertical-section/observation/hand-sample/sampling/anomaly/drill overlays, and the
