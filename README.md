...
Phase 7 alpha.1 now adds a source-gated greisen residual-fluid proxy around an evolved felsic pulse
through `/geology greisen` and the `greisen` review task. Its quartz-muscovite, tourmaline-proxy,
and kaolinitic-margin horizons are bounded projections rather than Sn/W assays or voxel-grade
inventories; the artifact records the closed proxy ledger and adjacent-chunk seam proof.
...
- `neoforge-adapter` is the pinned NeoForge 21.1.249 loader boundary. It contains the mod entry
  ...
  projection, bounded Phase 6 secondary-weathering, laterite, secondary-placer, paleosurface, and opt-in glacial overlays, the Phase 7 greisen residual-fluid proxy overlay, read-only `/geology` column/map/section/vertical-section/observation/hand-sample/sampling/anomaly/drill overlays, and the
