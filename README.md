...
The repository contains the **Phase 0 standalone geological atlas proof**, the **Phase 1 platform-neutral query core**, and the first three **Phase 2 petrologic material-state increments**. It intentionally produces no Minecraft blocks: deterministic random-access geology, validated scientific identity, explainable point queries, transient column/run plans, body-scale bulk-rock mineralogy and physical properties, and coarse element-reservoir conservation are proven before coupling the model to NeoForge.
