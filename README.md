...
Phase 9 alpha.1 now publishes condition-qualified element affinity, mobility, and host metadata for
the current material vocabulary. It is reviewable routing evidence rather than a fixed affinity
lookup, assay, or equilibrium calculation; expanded elements, reviewed partition data, and
processing-facing assay APIs remain separate increments.

Phase 9 alpha.2 expands the typed vocabulary to 50 elements, carries the new pathfinders through
the sparse trace vector and selected residual-inventory split, and preserves exact closure without
inventing trace mass in existing recipes. Solid-solution/polymorph, isotope/provenance, reviewed
partition, and processing-facing assay increments remain separate.

Phase 9 alpha.3 adds bounded solid-solution composition envelopes and deterministic serpentine
polymorph windows to the review artifact. These are inspectable routing limits around the ideal
interpolation model, not activity-coefficient or equilibrium calculations; isotope/provenance,
reviewed partition, and processing-facing assay increments remain separate.

Phase 9 alpha.4 adds optional K–Ar, Rb–Sr, Th–He, and U–He provenance evidence with exact
fixed-point parent/daughter-potential closure and source-age linkage. It is an auditable accumulated
potential proxy, not radiometric dating or daughter transport; reviewed partition and
processing-facing assay increments remain separate.

Phase 9 alpha.5 centralizes the sparse magma partition-response proxies in a versioned catalog,
wires residual-inventory capture through that table, and exposes confidence and review status in the
artifact. The rows remain authored proxies pending external scientific/license review; a
processing-facing assay/mineral-liberation API remains separate.

Phase 9 alpha.6 adds a deterministic processing-facing `ProcessingAssay`: resolved element totals
are allocated back to density/formula-derived modal hosts with exact fixed-point closure, including
non-crystalline hosts. The exposed ideal constituent-separation upper bound is review evidence,
not recovery, economic grade, or reserve; grain texture and process assumptions remain deferred to
Phase 10. Response and host-allocation rows remain authored pending external scientific/license
review.

Phase 9 alpha.7 adds a versioned partition-response review manifest. It identifies the
project-authored proxy source and records EarthRef/GERM as an external comparison candidate with
`REDISTRIBUTION_UNVERIFIED` licensing until a human review selects and approves rows. No third-party
dataset is bundled or silently promoted to runtime truth.
...
See [the Phase 0 architecture](docs/architecture/phase-0-atlas.md), [Phase 1 query-core increment](docs/architecture/phase-1-query-core.md), [Phase 2 material-state increment](docs/architecture/phase-2-material-state.md), [Phase 5 exploration increment](docs/architecture/phase-5-exploration.md), [Phase 6 secondary-systems increment](docs/architecture/phase-6-secondary-systems.md), [Phase 7 deposit-family increment](docs/architecture/phase-7-deposit-families.md), [Phase 8 dimension-profile increment](docs/architecture/phase-8-dimension-profiles.md), [Phase 9 geochemistry increment](docs/architecture/phase-9-geochemistry.md), [registry authoring contract](docs/architecture/registry-authoring.md), [reproducibility contract](docs/architecture/reproducibility.md), and [toolchain policy](docs/development/toolchain.md) before extending the model.
