...
/** Builds a bounded greisen overlay from the existing evolved felsic pulse and fluid evidence. */
public final class OverworldGreisenPlanner {
  private static final int PROFILE_PADDING_BLOCKS = 2;
  private final OverworldRegolithPlanner regolith;

  private OverworldGreisenPlanner(OverworldRegolithPlanner regolith) {
    this.regolith = regolith;
  }

  public static OverworldGreisenPlanner from(OverworldRegolithPlanner regolith) {
    return new OverworldGreisenPlanner(Objects.requireNonNull(regolith, "regolith planner"));
  }

  public OverworldGreisenColumnPlan plan(long blockX, long blockZ) {
    ...
    if (!province.id().equals(base.terrainControls().provinceId())
      ...
      throw new IllegalStateException("greisen and terrain owners changed between stages");
    }
    PetrologicSample parent = regolith.material().resolve(province, surface.surface().bedrock());
    GreisenSystemState system =
        GreisenSystemState.proofFor(
    ...
    List<OverworldGreisenInterval> intervals =
        system.status() == FormationStatus.FORMED
            ...
            : List.of();
    return new OverworldGreisenColumnPlan(
        blockX,
        blockZ,
        base.minYInclusive(),
        base.maxYExclusive(),
  ...
  public List<OverworldGreisenColumnPlan> planTargetChunk() {
    ChunkBlockBounds bounds = regolith.context().targetBounds();
    List<OverworldGreisenColumnPlan> columns = new ArrayList<>(256);
  ...
  private static List<OverworldGreisenInterval> classify(
    ...
    List<OverworldGreisenInterval> intervals = new ArrayList<>();
    GreisenSystemState.Horizon previous = null;
    ...
    for (int blockY = profileMin; blockY < profileMax; blockY++) {
      ...
      GreisenSystemState.Horizon current =
          system.zoneAt(province.frame().toLocal(worldPoint)).orElse(null);
      if (sameHorizon(previous, current)) {
        continue;
      }
      if (previous != null) {
        intervals.add(new OverworldGreisenInterval(intervalStart, blockY, previous));
      }
      previous = current;
      intervalStart = blockY;
    }
    if (previous != null) {
      intervals.add(new OverworldGreisenInterval(intervalStart, profileMax, previous));
    }
    return List.copyOf(intervals);
  }

  private static boolean sameHorizon(
      GreisenSystemState.Horizon first, GreisenSystemState.Horizon second) {
    return first == second
        || (first != null
            && second != null
