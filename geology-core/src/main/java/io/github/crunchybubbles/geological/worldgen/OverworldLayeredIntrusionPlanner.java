...
/** Builds chunk-local layered-intrusion overlays from explicit chamber evidence. */
public final class OverworldLayeredIntrusionPlanner {
  private static final int PROFILE_PADDING_BLOCKS = 2;
  private final OverworldRegolithPlanner regolith;
  private final LayeredIntrusionHostPolicy hostPolicy;

  private OverworldLayeredIntrusionPlanner(
      OverworldRegolithPlanner regolith, LayeredIntrusionHostPolicy hostPolicy) {
    this.regolith = regolith;
    this.hostPolicy = hostPolicy;
  }

  /** Uses actual resolved bedrock only; no layered chamber is inferred from generic mafic rock. */
  public static OverworldLayeredIntrusionPlanner from(OverworldRegolithPlanner regolith) {
    return from(regolith, LayeredIntrusionHostPolicy.none());
  }

  public static OverworldLayeredIntrusionPlanner from(
      OverworldRegolithPlanner regolith, LayeredIntrusionHostPolicy hostPolicy) {
    return new OverworldLayeredIntrusionPlanner(
        Objects.requireNonNull(regolith, "regolith planner"),
        Objects.requireNonNull(hostPolicy, "layered intrusion host policy"));
  }

  public OverworldLayeredIntrusionColumnPlan plan(long blockX, long blockZ) {
    ...
    if (!province.id().equals(base.terrainControls().provinceId())
      ...
      throw new IllegalStateException(
          "layered intrusion and terrain owners changed between stages");
    }
    ...
    LayeredIntrusionSystemState system =
        LayeredIntrusionSystemState.proofFor(
    ...
    List<OverworldLayeredIntrusionInterval> intervals =
        system.status() == FormationStatus.FORMED
            ...
            : List.of();
    return new OverworldLayeredIntrusionColumnPlan(
        blockX,
        blockZ,
        base.minYInclusive(),
        base.maxYExclusive(),
  ...
  public List<OverworldLayeredIntrusionColumnPlan> planTargetChunk() {
    ChunkBlockBounds bounds = regolith.context().targetBounds();
    List<OverworldLayeredIntrusionColumnPlan> columns = new ArrayList<>(256);
  ...
  public LayeredIntrusionHostPolicy hostPolicy() {
    return hostPolicy;
  }

  private static List<OverworldLayeredIntrusionInterval> classify(
    ...
    List<OverworldLayeredIntrusionInterval> intervals = new ArrayList<>();
    LayeredIntrusionSystemState.Horizon previous = null;
    ...
    for (int blockY = profileMin; blockY < profileMax; blockY++) {
      ...
      LayeredIntrusionSystemState.Horizon current =
          system.zoneAt(province.frame().toLocal(worldPoint)).orElse(null);
      if (sameHorizon(previous, current)) {
        continue;
      }
      if (previous != null) {
        intervals.add(new OverworldLayeredIntrusionInterval(intervalStart, blockY, previous));
      }
      previous = current;
      intervalStart = blockY;
    }
    if (previous != null) {
      intervals.add(new OverworldLayeredIntrusionInterval(intervalStart, profileMax, previous));
    }
    return List.copyOf(intervals);
  }

  private static boolean sameHorizon(
      LayeredIntrusionSystemState.Horizon first, LayeredIntrusionSystemState.Horizon second) {
    return first == second
        || (first != null
            && second != null
